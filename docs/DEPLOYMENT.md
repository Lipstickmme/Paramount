# Deploying Paramount Logistics to Vercel
...
| `/api/*` (tracking, shipments, quotes, contact, chat, inbound mail) | Vercel serverless functions (`api/[...path].js` and the nested re-exports) |
| Consignments, movements, rate requests, enquiries, chat, inbound mail | Supabase (Postgres) |
...
All current artwork is generated placeholder SVG. Every image on the site is a
slot, and a slot takes a real photograph with no code change at all: **drop the
file into `public/assets/img/` under one of the names below** and the next build
picks it up. Any of `.webp`, `.avif`, `.jpg`, `.jpeg`, `.png` works, and `.webp`
is chosen first if more than one is present.

| Name | Where it appears | Suggested size |
| ---- | ---------------- | -------------- |
| `hero-1`, `hero-2`, `hero-3` | The slideshow beside the tracking console | 1920 x 1080 |
| `paramount-underlay` | Fixed behind every page | 2400 x 1400 |
| `paramount-about` | The About page | 1600 x 1000 |
| `paramount-network` | The Network page | 1600 x 1000 |
| `paramount-control` | The control-tower block on the home page | 1600 x 1000 |
| `paramount-ceo` | Leadership portrait | 1200 x 1400 (4:5) |
| `paramount-careers` | The Careers page | 1600 x 1000 |
| `paramount-contact` | The Contact page | 1600 x 1000 |
| `paramount-og` | The social preview card | 1200 x 630 |

Service cards take `service-air`, `service-ocean`, `service-road`,
`service-rail`, `service-express` and `service-warehousing`, at about
1200 x 900.

`paramount-underlay` carries the most weight: it is fixed behind the whole site,
so pick the one that reads as the company's signature and has calm space rather
than busy detail in the middle, where headlines sit.

The mapping lives in `src/data/images.json`. Each slot names the files it prefers
...
**Logo.** Replace `public/assets/brand/paramount-wordmark.svg` (for the dark
theme), `paramount-wordmark-dark.svg` (for the light theme) and `/favicon.svg`.

Notes:

- Names are matched without regard to case, so `Hero-1.PNG` and `hero-1.png`
...
- Run `npm run build` after adding files, or let Vercel do it on deploy. The
  build prints which real images it found.
- `node scripts/make-placeholders.js` regenerates the placeholder set if you
...
The function logs `[paramount] storage: Supabase` on the next request. Confirm what the
...
3. In Vercel, add:
   ...
   | `FORM_FROM` | `Paramount Logistics <onboarding@resend.dev>` to start |
...
`Paramount Logistics <ops@yourdomain.com>`. Mail from a verified domain is far less
...
This gives you `ops@yourdomain.com` that lands in whatever inbox you actually read.
The endpoint is `POST /api/inbound/resend`.

1. In Resend, verify your domain if you have not already, and enable **inbound email**
   for the address you want (for example `ops@yourdomain.com`), following Resend's
   DNS instructions.
...
4. In Vercel, add:
   ...
   | `MAILBOX_ADDRESS` | `ops@yourdomain.com` |
